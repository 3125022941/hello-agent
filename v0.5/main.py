...
from schemas import EventCreate, ObservationCreate, TaskCreate,ReviewCreate,OpportunityGenerateCreate
...
#查看所有复盘
@app.get("/reviews")
def list_reviews_api(session: Session = Depends(get_session)):
    items = list_reviews(session)
...
def generate_opportunity_api(
    data:OpportunityGenerateCreate,
    session:Session=Depends(get_session),
):
    result=generate_opportunity(data,session)
    return result